...
export class ProjectDatabaseService {
  ...
  async createProjectDatabase(
    ...
    try {
      ...
      // Use dollar-quoted string to safely pass password (prevents SQL injection)
      // Note: Password is validated/generated by the platform, not user input
      await pool.query(
        `CREATE ROLE ${authenticatorRole} LOGIN PASSWORD $$${dbPassword}$$ IN ROLE anon, authenticated, service_role`
      );
